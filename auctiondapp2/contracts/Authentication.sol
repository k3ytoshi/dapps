...
contract Authentication is Ownable, Killable, ReentryProtector {
    ...
    function login() 
        ...
        returns (bytes32, bytes32, bytes32, string memory, UserType, UserState) 
    {
        if (owner == msg.sender) 
        {
            return (
    ...
    */
    function update(
        ...
        // onlyValidProfilePicture(_profilePicture)
        onlyExistingUser(msg.sender)
        returns (bytes32, bytes32, bytes32, string memory) 
    {
        externalEnter();
    ...
    */
    function getUser(uint _id) 
        ...
        returns ( address, bytes32, bytes32, bytes32, string memory, UserType, UserState) 
    {
